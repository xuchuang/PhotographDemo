...
- (void)customUI{
    ...
    _PhotoButton.frame = CGRectMake(kScreenWidth*1/2.0-50, kScreenHeight-100, 60, 60);
    ...
    _slider.frame =CGRectMake(15, _PhotoButton.frame.origin.y - 30, 220, 20);
    ...
    //设置滑块的颜色
    _slider.thumbTintColor=[[UIColor greenColor] colorWithAlphaComponent:0.8];
    
    //对滑动条添加事件函数
    [_slider addTarget:self action:@selector(settingExposure:) forControlEvents:UIControlEventValueChanged];
    
    [self.view addSubview:_slider];
    

    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(_slider.frame) + 10,_PhotoButton.frame.origin.y - 30,60, 20)];
...
- (void)settingExposure:(UISlider *)slider{
    
    AVCaptureDeviceFormat *format =  self.device.activeFormat;
    CGFloat iso = (format.maxISO - format.minISO) * _slider.value+ format.minISO;
    NSError *error;
    [self.device lockForConfiguration:&error];
    [self.device setExposureModeCustomWithDuration:AVCaptureExposureDurationCurrent ISO:iso completionHandler:^(CMTime syncTime) {
